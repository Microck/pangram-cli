//! Converts the approved Pangram fox GIF and cat-pufferfish still into
//! deterministic terminal cells.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let pufferfish_bytes = fs::read(PUFFERFISH_SOURCE_PATH)?;
    require(
        Sha256Hash::digest(&pufferfish_bytes).to_string() == PUFFERFISH_SOURCE_SHA256,
        "approved cat-pufferfish GIF hash does not match metadata",
    )?;
    let pufferfish_source = pufferfish::decode(&pufferfish_bytes)?;
    ...
    let outputs = [
        (
            OUTPUT_PATH,
            render_module(
                &format!("Source SHA-256: {SOURCE_SHA256}"),
                ART_WIDTH,
                ART_HEIGHT,
                &fox_frames,
                &playback_sequence(),
    ...
    for (path, generated) in outputs {
        if check_only {
            require(
                fs::read(path)?.as_slice() == generated.as_bytes(),
...
/// The dissolve phase at which a cell clears; the final phase clears all.
fn dissolve_score(x: usize, y: usize) -> usize {
    (x * 5 + y * 3 + (x * y) % 7) % DISSOLVE_FRAME_COUNT
}
...
fn render_module(
    ...
    writeln!(output, "pub(crate) const ART_WIDTH: usize = {width};").unwrap();
    writeln!(output, "pub(crate) const ART_HEIGHT: usize = {height};").unwrap();
    writeln!(
        output,
    ...
    for frame in frames {
        ...
        for row in frame.chunks_exact(width) {
