...
/// Adapter-facing lifecycle evidence for a combined analysis's asynchronous
/// AI-detection member. Acceptance always precedes progress for the same ID.
pub enum CombinedAnalysisObservation<'a, C = super::config::SystemClock> {
    Accepted(&'a RunningAnalysis<C>),
...
fn submission_unknown_error(
    ...
    submission_unknown_for(
        ...
        "task creation unacknowledged",
    )
}

fn submission_unknown_for(
    analysis_id: AnalysisId,
    request_sha256: crate::domain::Sha256Hash,
    last_status: &str,
) -> CanonicalError {
    let details = SubmissionOutcomeUnknownDetails::new(
        LocalOperationId::AnalysisId(analysis_id),
        request_sha256,
        None,
        None,
        NonEmptyString::new(last_status.to_owned()).expect("fixed label is non-empty"),
    );
