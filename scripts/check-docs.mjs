...
const landingFile = join(docsRoot, 'public/landing.html');
...
async function files(directory) {
  ...
  for (const entry of entries) {
    ...
    else if (['.md', '.mdx', '.html', '.ts', '.tsx', '.js', '.mjs', '.json', '.css'].includes(extname(path))) found.push(path);
  }
  return found;
}
...
failures.push(
  ...(await checkDocumentationLinks(externalLinkFiles, {
    displayRoot: root,
    sitePages: [{ path: '/', file: landingFile }],
    siteRoots: [
...
for (const path of docsFiles) {
  ...
  const commandText = extname(path) === '.html' ? htmlCodeText(text) : text;
  for (const match of commandText.matchAll(/^[ \t]*(?:\$\s+)?pangram ([a-z][a-z-]*)(?:\s|$)/gm)) {
...
for (const route of ['app/docs/layout.tsx', 'app/docs/[[...slug]]/page.tsx']) {
  await access(join(docsRoot, route)).catch(() => failures.push(`missing route ${route}`));
}
await access(landingFile).catch(() => failures.push('missing root landing page'));
